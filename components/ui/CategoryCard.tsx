    ...
    return (
      <div className="h-100 w-80 shrink-0 rounded-4xl overflow-hidden bg-cover bg-center relative group shadow-md">
        <div className="absolute inset-0 bg-black/20 group-hover:bg-black/30 transition-colors">
          <Image src={image} alt={name} fill className="object-cover bg-white w-full h-full" />
          <div className="bg-linear-to-t from-black/80 to-transparent absolute w-full h-full "></div>
</div>
    ...
    <div className="h-90 w-70 shrink-0 relative group shadow-md overflow-hidden">
      <div className="flex flex-col justify-center items-center h-full gap-4">
        <div className="h-full w-full bg-white rounded-2xl overflow-clip">
            <Image src={image} alt={name} height={300} width={300} className="object-cover bg-white w-full h-full rounded-2xl" />
        </div>
        <div className="px-4 p-2 h-max w-full bg-white rounded-2xl">
            <p className="text-stone-900 text-center font-medium">
                {name}
            </p>
        </div>
      </div>
